  ...
  return (
    <div className={cn("relative overflow-hidden border border-line bg-ink p-3 text-paper sm:p-5", className)}>
      <div className="mb-4 flex items-start justify-between gap-3 font-sans text-[10px] font-bold uppercase tracking-[0.16em] text-paper/60">
        <div><span className="text-evidence-blue">Search signal field</span><span className="mx-2 text-paper/25">/</span>Organic discovery</div>
        <div className="text-right">Live sample<br /><span className="text-evidence-violet">+34.2%</span></div>
      </div>
      <svg className="block h-auto w-full" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-labelledby="search-signal-title search-signal-description">
        ...
        <motion.polyline fill="none" points={impressions} stroke="#4285F4" strokeWidth="3" vectorEffect="non-scaling-stroke" initial={reducedMotion ? false : { pathLength: 0 }} animate={reducedMotion ? undefined : { pathLength: 1 }} transition={transition} />
        <motion.polyline fill="none" points={clicks} stroke="#673AB7" strokeWidth="3" vectorEffect="non-scaling-stroke" initial={reducedMotion ? false : { pathLength: 0 }} animate={reducedMotion ? undefined : { pathLength: 1 }} transition={{ ...transition, delay: reducedMotion ? 0 : 0.14 }} />
        {points.filter((_, index) => index % 8 === 3).map((point, index) => {
          ...
          return <g key={point.x}><circle cx={x} cy={y} r="4" fill="#4285F4" /><text x={x} y={y - 12} textAnchor="middle" className="fill-paper font-sans text-[9px] font-bold uppercase tracking-[0.1em]">Q{index + 1}</text></g>;
        })}
        <g className="font-sans text-[10px] font-bold uppercase tracking-[0.12em]">
          <circle cx={PADDING.left} cy="18" r="4" fill="#4285F4" /><text x={PADDING.left + 10} y="22" className="fill-paper/70">Impressions</text>
          <circle cx={PADDING.left + 116} cy="18" r="4" fill="#673AB7" /><text x={PADDING.left + 126} y="22" className="fill-paper/70">Clicks</text>
        </g>
      </svg>
